...
type automationDispatchAction struct {
	deviceID  int64
	action    string
	ordinal   int
	household int64
...
func (r *Repository) enqueueAutomationActions(ctx context.Context, householdID, automationID, runID int64, availableAt time.Time, actions []automationDispatchAction) error {
	return r.DB.WithTx(ctx, func(txctx context.Context) error {
		executor := r.executor(txctx)
		for _, item := range actions {
			if _, err := executor.ExecContext(txctx, `INSERT INTO outbox_messages(household_id,topic,payload,available_at) VALUES($1,'device.command',jsonb_build_object('automation_id',$2::bigint,'run_id',$3::bigint,'device_id',$4::bigint,'action',$5::text,'ordinal',$6::int),$7)`, householdID, automationID, runID, item.deviceID, item.action, item.ordinal, availableAt); err != nil {
				return err
			}
		}
		return nil
