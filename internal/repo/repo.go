...
func (r *Repository) CreatePlan(ctx context.Context, p model.EnergyPlan, devices []model.PlanDevice) (model.EnergyPlan, error) {
	...
	ex := r.executor(ctx)
	if err := ex.QueryRowContext(ctx, `INSERT INTO energy_plans(household_id,name,state,budget_cents,starts_at,ends_at) VALUES($1,$2,$3,$4,$5,$6) RETURNING id,household_id,name,state,budget_cents,starts_at,ends_at,created_at`, p.HouseholdID, p.Name, model.PlanDraft, p.BudgetCents, p.StartsAt, p.EndsAt).Scan(&out.ID, &out.HouseholdID, &out.Name, &out.State, &out.BudgetCents, &out.StartsAt, &out.EndsAt, &out.CreatedAt); err != nil {
		return model.EnergyPlan{}, classifyWriteError(err)
	}
	for _, d := range devices {
		result, err := ex.ExecContext(ctx, `INSERT INTO plan_devices(plan_id,device_id,target_watts) SELECT $1,id,$3 FROM devices WHERE id=$2 AND household_id=$4 AND state='enabled'`, out.ID, d.DeviceID, d.TargetWatts, p.HouseholdID)
		if err != nil {
			return model.EnergyPlan{}, classifyWriteError(err)
		}
		affected, err := result.RowsAffected()
		if err != nil {
			return model.EnergyPlan{}, err
		}
		if affected != 1 {
			return model.EnergyPlan{}, fmt.Errorf("%w: plan device %d is not enabled in household %d", model.ErrConflict, d.DeviceID, p.HouseholdID)
		}
	}
	return out, nil
}
