...
func (r *Repository) CreateDevice(ctx context.Context, d model.Device, capabilities []string) (model.Device, error) {
	...
	err := r.DB.WithTx(ctx, func(txctx context.Context) error {
		...
		for _, capability := range capabilities {
			if _, e := ex.ExecContext(txctx, `INSERT INTO device_capabilities(device_id,capability) VALUES($1,$2)`, out.ID, capability); e != nil {
				return e
			}
		}
		return nil
...
func (r *Repository) InsertTelemetry(ctx context.Context, t model.Telemetry) error {
	_, err := r.executor(ctx).ExecContext(ctx, `INSERT INTO telemetry(device_id,sequence,power_watts,temperature_c,measured_at) VALUES($1,$2,$3,$4,$5)`, t.DeviceID, t.Sequence, t.PowerWatts, t.TemperatureC, t.MeasuredAt)
	return err
}
...
func (r *Repository) CreatePlan(ctx context.Context, p model.EnergyPlan, devices []model.PlanDevice) (model.EnergyPlan, error) {
	...
	err := r.DB.WithTx(ctx, func(txctx context.Context) error {
		...
		for _, d := range devices {
			if _, e := ex.ExecContext(txctx, `INSERT INTO plan_devices(plan_id,device_id,target_watts) VALUES($1,$2,$3)`, out.ID, d.DeviceID, d.TargetWatts); e != nil {
				return e
			}
		}
		return nil
...
func (r *Repository) SetPlanState(ctx context.Context, id int64, from, to model.PlanState) error {
	res, err := r.executor(ctx).ExecContext(ctx, `UPDATE energy_plans SET state=$2 WHERE id=$1 AND state=$3`, id, to, from)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
...
func (r *Repository) CreateAutomation(ctx context.Context, a model.Automation, actions []model.AutomationAction) (model.Automation, error) {
	...
	err := r.DB.WithTx(ctx, func(txctx context.Context) error {
		...
		for _, action := range actions {
			if _, e := ex.ExecContext(txctx, `INSERT INTO automation_actions(automation_id,device_id,action,ordinal) VALUES($1,$2,$3,$4)`, out.ID, action.DeviceID, action.Action, action.Ordinal); e != nil {
				return e
			}
		}
		return nil
...
func (r *Repository) FinishRun(ctx context.Context, id int64, state model.RunState, cause string, now time.Time) error {
	_, err := r.executor(ctx).ExecContext(ctx, `UPDATE automation_runs SET state=$2,error_text=$3,finished_at=$4 WHERE id=$1`, id, state, cause, now)
	return err
}
...
func (r *Repository) ClaimOutbox(ctx context.Context) (model.OutboxMessage, error) {
	...
	err := r.executor(ctx).QueryRowContext(ctx, `WITH candidate AS (SELECT id FROM outbox_messages WHERE delivered_at IS NULL AND available_at <= now() ORDER BY id FOR UPDATE SKIP LOCKED LIMIT 1) UPDATE outbox_messages SET locked_at=now(),attempts=attempts+1 WHERE id=(SELECT id FROM candidate) RETURNING id,household_id,topic,payload,attempts,available_at,locked_at,delivered_at`).Scan(&m.ID, &m.HouseholdID, &m.Topic, &m.Payload, &m.Attempts, &m.AvailableAt, &m.LockedAt, &m.DeliveredAt)
	return m, err
}
func (r *Repository) MarkOutbox(ctx context.Context, id int64, delivered bool, next time.Time) error {
	if delivered {
		_, err := r.executor(ctx).ExecContext(ctx, `UPDATE outbox_messages SET delivered_at=now(),locked_at=NULL WHERE id=$1`, id)
		return err
	}
	_, err := r.executor(ctx).ExecContext(ctx, `UPDATE outbox_messages SET locked_at=NULL,available_at=$2 WHERE id=$1`, id, next)
	return err
}
