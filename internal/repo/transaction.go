...
func (r *Repository) insertDeviceCapabilities(ctx context.Context, deviceID int64, capabilities []string) error {
	for _, capability := range capabilities {
		if _, err := r.executor(ctx).ExecContext(ctx, `INSERT INTO device_capabilities(device_id,capability) VALUES($1,$2)`, deviceID, capability); err != nil {
