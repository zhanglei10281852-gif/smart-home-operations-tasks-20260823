...
func TestEnrollmentFailureLeavesNoPartialDevice(t *testing.T) {
	...
	if _, err := database.SQL.ExecContext(ctx, `DROP TRIGGER IF EXISTS reject_scene_capability ON device_capabilities; DROP FUNCTION IF EXISTS reject_scene_capability()`); err != nil {
		t.Fatal(err)
	}
	r := repo.New(database)
