...
func (r *Repository) planDevicesEligible(ctx context.Context, id int64) (bool, error) {
	var eligible bool
	err := r.executor(ctx).QueryRowContext(ctx, `SELECT NOT EXISTS (
		SELECT 1
		FROM plan_devices links
		JOIN devices ON devices.id=links.device_id
		JOIN energy_plans plans ON plans.id=links.plan_id
		WHERE links.plan_id=$1
		  AND (devices.state <> 'enabled' OR devices.household_id <> plans.household_id)
	)`, id).Scan(&eligible)
	return eligible, err
}
