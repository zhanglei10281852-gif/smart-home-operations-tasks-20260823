...
func (r *Repository) ExecuteAutomationRun(ctx context.Context, runID int64, finishedAt time.Time) error {
	...
	var actions []automationDispatchAction
	if err := r.DB.WithTx(ctx, func(txctx context.Context) error {
		...
		for rows.Next() {
			var item automationDispatchAction
	...
	if err := r.enqueueAutomationActions(ctx, householdID, automationID, runID, finishedAt, actions); err != nil {
		return fmt.Errorf("enqueue automation actions: %w", err)
	}
	executor := r.executor(ctx)
	requestID := fmt.Sprintf("automation-run-%d", runID)
	if _, err := executor.ExecContext(ctx, `INSERT INTO audit_events(household_id,request_id,object_type,object_id,action,payload) VALUES($1,$2,'automation_run',$3,'dispatched',jsonb_build_object('action_count',$4::int))`, householdID, requestID, fmt.Sprint(runID), len(actions)); err != nil {
		return fmt.Errorf("audit automation run: %w", err)
	}
	result, err := executor.ExecContext(ctx, `UPDATE automation_runs SET state='succeeded',error_text=NULL,finished_at=$2 WHERE id=$1 AND state='running'`, runID, finishedAt)
	if err != nil {
