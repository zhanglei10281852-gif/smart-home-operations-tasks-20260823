...
func (s *NotificationService) SendAlert(ctx context.Context, recipient, code, severity string) error {
	return domain.Send(ctx, s.Notifier, domain.Notification{Recipient: recipient, Channel: "push", Subject: "Smart home alert: " + code, Body: fmt.Sprintf("Severity: %s", severity)})
}
