...
func (d *DB) Health(ctx context.Context, now func() time.Time) (HealthReport, error) {
	...
	rows, err := d.queryServerVersion(ctx)
	if err != nil {
		return HealthReport{}, fmt.Errorf("query server version: %w", err)
	}
	var version string
	if !rows.Next() {
		...
		return HealthReport{}, errors.New("server version query returned no rows")
	}
